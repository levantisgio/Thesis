...
/****** Object:  StoredProcedure [TRG].[STG_TO_TRG_PBI]    Script Date: 11/5/2024 4:05:17 πμ ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [TRG].[STG_TO_TRG_PBI]
AS
BEGIN
    SET NOCOUNT ON;
	...
	WITH filter_new_data_cte AS (
        ...
        WHERE create_date > (
                ...
                FROM [TRG].[dataset01_pbi]
            )
		--OR create_date = '1900-01-01'
    )
    
    INSERT INTO [TRG].[dataset01_pbi] (
        [key_column],
        [timestamp],
        [create_date],
        [year],
